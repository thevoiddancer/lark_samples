...
class LarkTransformer(Transformer):
    def __default__(self, data, children, meta):
        if data in ['title']:
            return {'source name': ' '.join(children)}
        elif data in ['order', 'points', 'groups', 'songs', 'samples']:
            return {data.value: children[0]}
